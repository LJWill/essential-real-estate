					...
					foreach ( $property_gallery as $image ):
						...
						if ( ! empty( $image_src ) ) {
							...
							<div class="property-gallery-item ere-light-gallery">
								<img id="my-main" src="<?php echo esc_url( $image_src ) ?>" alt="<?php the_title(); ?>"
									title="<?php the_title(); ?>">
								<a data-thumb-src="<?php echo esc_url($image_full_src[0]); ?>" data-gallery-id="<?php echo esc_attr($gallery_id); ?>"
								data-rel="ere_light_gallery" href="<?php echo esc_url($image_full_src[0]); ?>" class="zoomGallery">
								<iclass="fa fa-expand"></i></a>
							</div>
						<?php } ?>
					<?php endforeach; ?>
				</div>
					...
					foreach ( $property_gallery as $image ):
						...
						if (! empty( $image_src ) ) {?>
							<div class="property-gallery-item">
								<img id="my-thumbnail" src="<?php echo esc_url( $image_src ) ?>" alt="<?php the_title(); ?>"
									title="<?php the_title(); ?>">
							</div>
						<?php } ?>
					<?php endforeach; ?>
